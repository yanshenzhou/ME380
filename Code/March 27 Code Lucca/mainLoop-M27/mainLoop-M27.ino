...
void loop() {
  ...
  delay(500);
  motorMove(false, 5.7);//move up in z all the way
  motorMove(true, 60);//move halfway to finish
  servoRotateArm.write(30);//align arm away from platform
  delay(500);
  motorMove(true, 58);//move die to finish
  ...
  delay(20000);
  motorMove(false, 1);
  motorMove(true, -118);//move die back to start
  motorMove(false, -5.3);
  delay(500);
  grip(false);
  delay(20000);
...
void orientModel() {
  ...
  motorMove(false, -2.8); //move back down
  grip(false);
  //int seenTopFace = 1;
...
void transformDie(int axis, int rotation) {
  ...
  delay(200);
  motorMove(false, -2.8);//move back down in z for 2 seconds
  delay(500);
  grip(false);//ungrip
}
